...
function instru(n){
    ...
    ti("INSTRUCTIONS<br>Pour valider clique sur Y,<br>Pour refuser clique sur N ")
        ...
        ti("INSTRUCTIONS<br>Pour valider clique sur Y, Pour refuser clique sur N ")
    }
}
...
function open_door1(){
    if (valeurs[ligne][colonne] === "_"){
        if (key<1){
            tz("Avant de franchir la porte, il faut récupérer une clé caché dans la map !")
        }
        else if (key === 1){
        ...
        if(code==="KeyE"){
            ...
            document.getElementById('mapzone').style.animation = "fadein 2s"
            alerte('Bonjour, je pense que vous cherchez la conseillère. Elle se situe dans la salle du haut')
            valeurs=valeurs2
            ligne = 18
            colonne = 10
            stepdor = 1 
...
function open_door2(){
    if (valeurs[ligne][colonne] === "~"){
        if(key<2 ){
            tz("Avant de franchir la porte il faut récupérer 2 clés cachées dans les maps !")
...
function open_door3(){
    if (valeurs[ligne][colonne] === "!"){
        if(key<3 ){
            tz("Avant de franchir la porte il faut récupérer 3 clés cachées dans les maps !")
...
function dialogue(){
    if (valeurs[ligne][colonne] === "*"){
        parole=parole+1
        if (parole === 1){
            dial("Bonjour et bienvenue à HETIC ! <br><br> Pour accéder aux salles secondaires, il te faut récupérer 3 clés que tu trouvera à différents endroit ...", "Pour décrocher ton stage et être éventuellement recruté, tu doit déposer ton CV dans 9 boites disponibles dans les différentes maps...")
        }else if (parole === 2){
            instru(0)
            tz("PNJ : As-tu besoin d'aide ?")
            document.addEventListener('keypress', (event) => {
                let code = event.code;
            if(code==="KeyY"){
            ...
            if(livres < 3){
                alerte('Attention je vois que tu n\'a pas déposé assez de CV dans la map précédente,<br> tu peut toujours y retourner.')
            }
...
function prompte(q,a1,a2,a3,r){
    ...
    function fin_jeu(){
        if (!(stp>=9 && livres>=9)){
            ...
            tz('Dommage t\'a candidature n\'a pas été retenu !')
            ti('Appui sur F5')
        }else{
            setTimeout(() => {document.getElementById('mapzone').style.animation = ""}, 2000)
